...
void w_CompGraph_eval()
{
    ...
    std::shared_ptr<breptopo::TopoNaming> tn = nullptr;
    auto v_hist = ves_toforeign(2);
    if (v_hist) {
        tn = ((wrapper::Proxy<breptopo::TopoNaming>*)v_hist)->obj;
    }
    
    int node_num = cg->GetGraph()->GetNodesNum();
    auto cvar = cnode.GetCompNode()->Eval(*cg, tn, node_idx);
    if (!cvar) {
        ves_set_nil(0);
        return;
    }
...
VesselForeignMethodFn BrepTopoBindMethod(const char* signature)
{
    ...
    if (strcmp(signature, "TopoNaming.get_edge_graph()") == 0) return w_HistMgr_get_edge_graph;
    if (strcmp(signature, "TopoNaming.get_face_graph()") == 0) return w_HistMgr_get_face_graph;
    if (strcmp(signature, "TopoNaming.get_solid_graph()") == 0) return w_HistMgr_get_solid_graph;
