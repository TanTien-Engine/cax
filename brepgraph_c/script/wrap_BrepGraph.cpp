#include "brepgraph_c/script/wrap_BrepGraph.h"
...
void w_TopoGraph_allocate()
{
    ...
    auto proxy = (wrapper::Proxy<brepgraph::TopoGraph>*)ves_set_newforeign(0, 0, sizeof(wrapper::Proxy<brepgraph::TopoGraph>));
    proxy->obj = std::make_shared<brepgraph::TopoGraph>(shapes);
}

int w_TopoGraph_finalize(void* data)
{
    auto proxy = (wrapper::Proxy<brepgraph::TopoGraph>*)(data);
    proxy->~Proxy();
    return sizeof(wrapper::Proxy<brepgraph::TopoGraph>);
}

void w_TopoGraph_get_graph()
{
    auto hg = ((wrapper::Proxy<brepgraph::TopoGraph>*)ves_toforeign(0))->obj;
...
{
    auto proxy = (wrapper::Proxy<brepgraph::TopoNaming>*)ves_set_newforeign(0, 0, sizeof(wrapper::Proxy<brepgraph::TopoNaming>));
    proxy->obj = std::make_shared<brepgraph::TopoNaming>();
}

int w_TopoNaming_finalize(void* data)
{
    auto proxy = (wrapper::Proxy<brepgraph::TopoNaming>*)(data);
    proxy->~Proxy();
    return sizeof(wrapper::Proxy<brepgraph::TopoNaming>);
}

void w_TopoNaming_get_vertex_graph()
{
    auto tn = ((wrapper::Proxy<brepgraph::TopoNaming>*)ves_toforeign(0))->obj;

    ves_pop(ves_argnum());

    ves_pushnil();
    ves_import_class("brepgraph", "HistGraph");
    auto proxy = (wrapper::Proxy<brepgraph::HistGraph>*)ves_set_newforeign(0, 1, sizeof(wrapper::Proxy<brepgraph::HistGraph>));
...
{
    auto proxy = (wrapper::Proxy<brepgraph::HistGraph>*)ves_set_newforeign(0, 0, sizeof(wrapper::Proxy<brepgraph::HistGraph>));
    proxy->obj = std::make_shared<brepgraph::HistGraph>();
}

int w_HistGraph_finalize(void* data)
{
    auto proxy = (wrapper::Proxy<brepgraph::HistGraph>*)(data);
    proxy->~Proxy();
    return sizeof(wrapper::Proxy<brepgraph::HistGraph>);
}

void w_HistGraph_get_hist_graph()
{
    auto hg = ((wrapper::Proxy<brepgraph::HistGraph>*)ves_toforeign(0))->obj;
...
void w_HistGraph_query_shapes()
{
    ...
    if (hg->QueryNodes(uid, nodes))
    {
        ...
        for (int i = 0; i < num; ++i)
        {
            ...
            proxy->obj = nodes[i]->GetComponent<brepgraph::NodeShape>().GetShape();
            ves_pop(1);
            ves_seti(-2, i);
            ves_pop(1);
        }
...
void w_CompGraph_eval()
{
    ...
    if (auto* v = std::get_if<int>(&result)) {
        ...
        ves_set_boolean(0, *v);
    } else if (auto* v = std::get_if<brepgraph::ShapeVal>(&result)) {
        if (v->shape) {
...
void w_CompGraph_add_number3_node()
{
    ...
    int id = cg->AddConst(brepgraph::Vec3{x, y, z}, desc);
    ves_set_number(0, id);
}
