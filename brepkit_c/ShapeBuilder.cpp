...
std::shared_ptr<TopoShape> ShapeBuilder::MakeEdge(const gs::Line3D& l)
{
...
std::shared_ptr<TopoShape> ShapeBuilder::MakeEdge(const gs::Arc3D& arc)
{
...
std::shared_ptr<TopoShape> ShapeBuilder::MakeEdge(const TrimmedCurve& c, const CylindricalSurface& s)
{
	TopoDS_Edge edge = BRepBuilderAPI_MakeEdge(c.GetShape(), s.GetShape());
	return std::make_shared<TopoShape>(edge);
}

std::shared_ptr<TopoShape> ShapeBuilder::MakeWire(const std::vector<std::shared_ptr<TopoShape>>& edges)
{
	TopoDS_Wire wire;
	if (edges.size() == 1) 
	{
...
std::shared_ptr<TopoShape> ShapeBuilder::MakeFace(const TopoShape& wire)
{
	TopoDS_Face face = BRepBuilderAPI_MakeFace(wire.ToWire());
	return std::make_shared<TopoShape>(face);
}

std::shared_ptr<TopoShape> ShapeBuilder::MakeShell(const std::vector<std::shared_ptr<TopoShape>>& faces)
{
	TopoDS_Shell shell;
...
std::shared_ptr<TopoShape> ShapeBuilder::MakeCompound(const std::vector<std::shared_ptr<TopoShape>>& shapes)
{
	TopoDS_Compound res;
	BRep_Builder builder;
