...
#include "brepkit_c/ShapeTools.h"
...
TEST_CASE("FindFaceKey and FindFaceIdx roundtrip") {
    ...
    auto face_key = ShapeTools::FindFaceKey(shape, 1);
    REQUIRE(face_key != nullptr);

    int idx = ShapeTools::FindFaceIdx(shape, face_key);
    CHECK(idx == 1);

    CHECK(ShapeTools::FindFaceKey(shape, 0) == nullptr);
    CHECK(ShapeTools::FindFaceKey(shape, 100) == nullptr);
}

TEST_CASE("FindEdgeKey and FindEdgeIdx roundtrip") {
    auto shape = make_box(10, 20, 30);

    auto edge_key = ShapeTools::FindEdgeKey(shape, 1);
    REQUIRE(edge_key != nullptr);

    int idx = ShapeTools::FindEdgeIdx(shape, edge_key);
    CHECK(idx == 1);
}

TEST_CASE("ShapeBuilder MakeCompound") {
    auto box1 = make_box(10, 20, 30);
    auto box2 = make_box(5, 5, 5);

    std::vector<std::shared_ptr<TopoShape>> shapes = { box1, box2 };
    auto compound = ShapeBuilder::MakeCompound(shapes);
...
TEST_CASE("FindFaceIdx returns 0 for unrelated face") {
    ...
    auto face_from_box2 = ShapeTools::FindFaceKey(box2, 1);
    REQUIRE(face_from_box2 != nullptr);

    int idx = ShapeTools::FindFaceIdx(box1, face_from_box2);
    CHECK(idx == 0);
}
